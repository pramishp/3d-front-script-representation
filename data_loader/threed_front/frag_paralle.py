...
# Load dataset (we'll only iterate here and pass scenes to worker processes)
...
def process_scene(scene):
    ...
    return fragmented_walls, non_fragmented_walls, scene_fragment_summary

if __name__ == "__main__":
    num_processes = 10  # Adjust based on your system and desired load
    pool = mp.Pool(processes=num_processes)
    results = pool.map(process_scene, d.scenes)
    pool.close()
    pool.join()

    all_fragmented_walls = []
    all_non_fragmented_walls = []
    all_scene_fragment_summary = {}

    for frag_walls, non_frag_walls, scene_summary in results:
        all_fragmented_walls.extend(frag_walls)
        all_non_fragmented_walls.extend(non_frag_walls)
        all_scene_fragment_summary.update(scene_summary)

    # Save results
    with open(os.path.join(output_dir, "fragmented_walls.json"), "w") as f:
        json.dump(all_fragmented_walls, f, indent=2)

    with open(os.path.join(output_dir, "non_fragmented_walls.json"), "w") as f:
        json.dump(all_non_fragmented_walls, f, indent=2)

    with open(os.path.join(output_dir, "scene_broken_wall_summary.json"), "w") as f:
        json.dump(all_scene_fragment_summary, f, indent=2)

    # Print counts
    count_fragmented = len(all_fragmented_walls)
    count_non_fragmented = len(all_non_fragmented_walls)

    print(f"\nAnalysis of fragmented and non-fragmented walls complete (parallel processing). Results saved in '{output_dir}'.")
    ...
    print(f"Number of scenes with at least one broken wall: {len(all_scene_fragment_summary)}")